...
const fetchAPI = async (endpoint, options = {}, token = null) => {
  ...
  // Deteksi apakah ini endpoint auth (login/register)
  const isAuthRoute =
    endpoint.includes("/auth/login") || endpoint.includes("/auth/register");

  // DETEKSI SESI HABIS:
  // Hanya trigger sesi habis jika BUKAN di halaman auth dan mendapat status 401
  ...
  if (isSessionExpired) {
    ...
    if (!window.isSessionAlertShown) {
      ...
      // PERBAIKAN: Gunakan BASE_URL agar dinamis mengikuti konfigurasi Vite
  ...
  // Handle error umum lainnya (termasuk 401 saat login salah password)
  if (!response.ok) {
...
// FITUR NON-MVP (DI-KOMEN SEMENTARA)
/*
...
// FITUR NON-MVP (DUMMY EXPORT AGAR VITE TIDAK CRASH)
// fungsi kosong agar komponen yang dinonaktifkan tetap bisa import tanpa error.
...
// [PERBAIKAN]: Ubah method dari PUT menjadi POST untuk Method Spoofing
...
export const updateAdminBook = (id, formData, token) =>
  fetchAPI(`/admin/books/${id}`, { method: "POST", body: formData }, token);

export const deleteAdminBook = (id, token) =>
  fetchAPI(`/admin/books/${id}`, { method: "DELETE" }, token);

// MANAJEMEN SCENE/HALAMAN SECARA TERPISAH

// Menambah 1 halaman ke buku tertentu
export const addAdminBookPage = (bookId, formData, token) =>
  fetchAPI(
    `/admin/books/${bookId}/pages`,
    { method: "POST", body: formData },
    token,
  );

// Mengupdate 1 halaman tertentu (Gunakan Method Spoofing karena ada file upload)
export const updateAdminBookPage = (pageId, formData, token) =>
  fetchAPI(`/admin/pages/${pageId}`, { method: "POST", body: formData }, token);

// Menghapus 1 halaman tertentu
export const deleteAdminBookPage = (pageId, token) =>
  fetchAPI(`/admin/pages/${pageId}`, { method: "DELETE" }, token);

export const getAdminProfile = (token) => fetchAPI("/admin/profile", {}, token);

// [PERBAIKAN]: Ubah method dari PUT menjadi POST untuk Method Spoofing
export const updateAdminProfile = (formData, token) =>
  fetchAPI("/admin/profile", { method: "POST", body: formData }, token);
...
// TAMBAH VIEW BUKU
export const incrementBookView = (id) =>
  fetchAPI(`/books/${id}/view`, { method: "POST" });
