...
const AdminProfileSettings = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // PERBAIKAN: Menerapkan Logic Fallback Avatar dengan Inisial Nama
        if (data.avatar_url) {
          setExistingAvatar(data.avatar_url);
          setAvatarPreview(getImageUrl(data.avatar_url));
        } else {
          setAvatarPreview(
            `https://placehold.co/150x150/F8AF2F/FFFFFF?text=${data.first_name?.charAt(0).toUpperCase() || "A"}`,
          );
        }
      } catch (err) {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      showSuccess(res.message || "Profil berhasil diperbarui!");
      if (res.avatar_url) {
        setExistingAvatar(res.avatar_url);
        setAvatarPreview(getImageUrl(res.avatar_url));
      }
    } catch (err) {
      showError("Gagal update profil: " + err.message);
    } finally {
      showLoading(false);
    }
  };
  ...
  const handleSavePassword = async (e) => {
    ...
    try {
      ...
      showSuccess(res.message || "Password berhasil diubah!");
