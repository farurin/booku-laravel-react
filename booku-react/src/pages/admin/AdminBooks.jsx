  ...
  useEffect(() => {
    const fetchBooks = async () => {
      ...
      try {
        ...
        showError("Gagal menarik data buku: " + error.message);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const statCards = [
    {
      ...
      count: reviewCount,
      label: "Proses Review",
      icon: <HiOutlineClock className="w-8 h-8 text-[#8f30ff]" />,
      bgIcon: "bg-[#fff8ef]",
      borderColor: "border-[#8f30ff]",
    },
    {
      id: 2,
      count: terbitCount,
  ...
  return (
    <div className="p-8 md:p-12 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center mb-10">
        ...
        <Link
          to="/admin/tambah"
          className="w-full sm:w-auto shrink-0 bg-[#8f30ff] hover:bg-[#dd2d8f] text-white font-bold px-6 py-3.5 rounded-xl flex items-center justify-center gap-2 transition shadow-sm cursor-pointer"
        >
            ...
            {isLoading ? (
              ...
              paginatedBooks.map((book, index) => (
                <tr
                  ...
                  className={`group hover:bg-purple-50/50 transition cursor-pointer ${index !== paginatedBooks.length - 1 ? "border-b border-gray-100" : ""}`}
                >
                  <td className="py-4 px-6 flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-900 mb-1 leading-snug group-hover:text-[#8f30ff] transition-colors">
                        {book.title_id}
                      </h4>
                      <p className="text-[11px] text-gray-500 line-clamp-2 leading-relaxed">
                        {book.description_id || "Tidak ada deskripsi."}
                      </p>
                    </div>
                  </td>
