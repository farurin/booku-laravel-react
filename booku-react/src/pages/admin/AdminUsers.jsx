...
const AdminUsers = () => {
  ...
  const renderRole = (role) => {
    if (role === "Super Admin")
      return <span className="text-[#744fcd] font-bold">{role}</span>;
    if (role === "Admin")
      return <span className="text-[#744fcd] font-bold">{role}</span>;
    if (role === "Editor")
      return <span className="text-[#1da1f3] font-bold">{role}</span>;
    return role;
  };
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
          ...
          <button
            ...
            className="bg-[#8f30ff] hover:bg-[#dd2d8f] text-white font-bold px-6 py-3.5 rounded-xl shadow-sm transition-colors flex items-center gap-2 shrink-0 cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
          <div className="bg-[#792dc1] rounded-3xl p-6 flex items-center gap-5 shadow-sm text-white">
          ...
          <div className="bg-[#dd2d8f] rounded-3xl p-6 flex items-center gap-5 shadow-sm text-white">
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shrink-0">
              <svg
                width="32"
                height="32"
          ...
          <div className="bg-[#1da1f3] rounded-3xl p-6 flex items-center gap-5 shadow-sm text-white">
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shrink-0">
              <svg
                width="32"
                height="32"
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-bold transition-colors border cursor-pointer ${
                  activeRoleFilter === role
                    ? "bg-white text-[#792dc1] border-[#dd2d8f] shadow-sm"
          ...
          <div className="flex items-center gap-4 shrink-0 font-bold text-gray-900 text-sm bg-white px-4 py-2 rounded-full border border-gray-100 shadow-sm">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-1 hover:text-[#792dc1] cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
              >
            ...
            <tbody>
              {isLoading ? (
                ...
                paginatedUsers.map((u, idx) => {
                  // PERBAIKAN: Menambahkan Fallback Inisial pada Render Tabel User
                  const fallbackUserAvatar = `https://placehold.co/150x150/F8AF2F/FFFFFF?text=${u.name?.charAt(0).toUpperCase() || "U"}`;
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-sm font-bold text-gray-800 flex items-center gap-3">
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.target.src = fallbackUserAvatar;
                          }}
                        />
                        {u.name}
                      </td>
                      ...
                      <td className="py-4 px-6 flex items-center justify-center gap-2">
                        <button
                          ...
                          className="p-2 bg-[#952dff] hover:bg-yellow-500 text-white rounded-lg shadow-sm transition-colors cursor-pointer"
                          title="Edit Pengguna"
                        >
                          <HiPencil />
                        </button>
