...
import {
  ...
  HiDownload,
  HiX,
  HiChevronRight as HiArrowRight,
} from "react-icons/hi";
...
const AdminPerpustakaan = () => {
  ...
  const filteredBooks = books.filter((book) => {
    ...
    if (activeCategoryId === "all") {
      ...
      const activeCatName = activeCatObj ? activeCatObj.name_id : "";
      const matchByName = book.category_name_id === activeCatName;
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-black text-gray-900">
                {activeCategoryId === "all"
                  ? "Semua Cerita"
                  : categories.find((c) => c.id == activeCategoryId)?.name_id}
              </h2>
          ...
          {selectedBook && (
            <div className="w-full xl:w-95 shrink-0 bg-white rounded-4xl p-6 shadow-xl border border-gray-100 xl:sticky xl:top-8 transition-all animate-fade-in z-10 flex flex-col">
              ...
              <div className="flex gap-5 mb-6">
                <img
                  src={getImageUrl(
                    selectedBook.image_id ||
                      selectedBook.image_en ||
                      selectedBook.image,
                  )}
                  alt="Cover"
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <p className="text-[11px] font-bold italic text-orange-500 mb-3">
                    {selectedBook.category_name_id || "Tanpa Kategori"}
                  </p>

                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-xs font-bold text-gray-600">
                      <HiDownload className="text-green-500 text-sm" />{" "}
                      {selectedBook.views_count || 0} Dibaca
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-gray-600">
                      <HiBookmark className="text-blue-500 text-sm" />{" "}
                      {selectedBook.saved_count || 0} Disimpan
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-gray-600">
                      <HiHeart className="text-red-500 text-sm" />{" "}
                      {selectedBook.favorites_count || 0} Disukai
                    </div>
                  </div>
                </div>
              </div>
