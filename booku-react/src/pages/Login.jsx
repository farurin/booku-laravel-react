...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validation.isValid) {
      setIsError(true);
      setErrorMessage(t("auth_login_err_val"));
      return;
    }
    ...
    try {
      ...
      setIsError(true);
      setErrorMessage(t("auth_login_err_cred"));
    }
  };
  ...
  return (
    <AuthLayout>
      <div className="mb-10 text-left">
        ...
        <p className="text-gray-500 mt-2 text-sm">{t("auth_login_desc")}</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 py-3 border border-gray-200 rounded-xl hover:bg-gray-50 transition shadow-sm"
        >
          <IconGoogle />
        ...
        <button
          type="button"
          onClick={() => setIsModalOpen(true)}
          className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#1877F2] rounded-xl hover:bg-blue-600 transition shadow-sm"
        >
          <IconFacebook />
          <span className="text-sm text-white font-bold">Facebook</span>
        </button>
      </div>
